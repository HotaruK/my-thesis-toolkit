...
def marge_tensor(base_obj, additional_obj):
    new_sign = torch.cat((additional_obj['sign'], base_obj['sign']), dim=1)
...
def _process_dataset(base_ds_path, output_dir, additional_dataset_path, type, prefix, process='merge', key_name=None):
    assert process in ['merge', 'add']
    assert (process == 'add' and key_name)
